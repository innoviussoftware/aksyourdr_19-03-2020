...
class HomeController extends Controller
{
    ...
    public function labreportstypearray(Request $request)
    {
        ...
        $investigation=Investigation::where('type_id','4')->pluck('id')->toArray();
    ...
    public function xrayreportsarray(Request $request)
    {
        ...
        $investigation=Investigation::where('type_id','3')->pluck('id')->toArray();
    ...
    public function emrAdd(Request $request,$id)
    {
        ...
        return view('front.emr.EmrAdd',['medicines'=>$medicines,'investigations'=>$investigations,'specialitys'=>$specialitys,'emr_number'=>isset($users->emr_number)?$users->emr_number:'','type_visit'=>$type_visit,'patient_id'=>$id]);
    }
    ...
    public function emrStore(Request $request)
    {
            ...
            if(isset($investigation))
            {
                ...
                foreach ($investigation as $key => $e) {
                    ...
                    $investigation_name=Investigation::select('testname_english','type_name')->where('id',$e)->first();
                  ...
                  //   echo "https://localhost/asknew/".$path;
            ...
            if($investigation && $medicines || $investigation || $medicines )
            {
    ...
    public function storeUserEmr(Request $request){
        ...
        if ($validator->fails()) {
            ...
            //Payment Functions
                ...
                $commission=$fees*$commission_final/100;
                ...
                {   
                    ...
                    $update_patient_wallet_money=$patient_wallet_money-$fees;//Update Wallet Money Patient
                    ...
                    if($clinic)
                    {
                        ...
                        if($clinic_name=='Ask' || $clinic_name=='ask')
                        {
                            ...
                            $update_clinic_money=$clinic_wallet_money+$commission;

                            $clinic_wallet=Clinic::where('id',isset($clinic_wallet)?$clinic_wallet->clinic_id:'')->update(['wallet_money'=>$update_clinic_money]);//Update Patient 
                        ...
                        else
                        {
                            ...
                            $update_askmoney=$clinic_wallet_money+$commission;
                ...
                $payment_history->price = $fees;
                $payment_history->message = "Pay fees of doctor SAR".$fees;
        ...
        if(isset($investigation))
        {
                ...
                foreach ($investigation as $key => $e) {
                    if($e != null)
                    {
                        ...
                        $visit_investigation->investigation_id = $e ? $e :"";
                        $investigation_name=Investigation::select('testname_english')->where('id',$e)->first();
                        $visit_investigation->note = $notes[$key]?$notes[$key]:"";
                        $visit_investigation->investigation_name = $investigation_name->testname_english;
                        $visit_investigation->patient_id = request('patient_id') ? request('patient_id'): "";
                        $visit_investigation->doctor_id = request('doctor_id') ?request('doctor_id'):"";
    ...
    public function chatmessages(Request $request,$id)
    {
        ...
        if($receptorUser == null) {
            ...
            return view('front.user.chat', compact('receptorUser', 'chat','user','receiver_id','sender_name','sender_id'));
        }
    }
...
// chat
    ...
    public function videocallArray(Request $request)
    {
        ...
        $patient=VideoCallDocumentDetail::where('to_id',$id)->orwhere('from_id',$id)->get();
        ...
        foreach ($patients as $patient) {
                ...
                $patient_name=User::where('id',$patient['from_id'])->first();

                $sub[] = ($patient_name) ? ucfirst($patient_name->first_name).' '.$patient_name->last_name: "-";

                $sub[]='<a href="'.env('APP_URL_WITHOUT_PUBLIC').$patient['document'].'" target="_"><i class="fa fa-file-pdf-o"></i></a>';
